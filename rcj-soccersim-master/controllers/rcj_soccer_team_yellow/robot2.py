...
class MyRobot2(RCJSoccerRobot):
    def readData(self):
        ...
        self.heading = degrees(self.get_compass_heading()
                               )  # خواندن سنسور قطب نما
        ...
        if self.is_new_ball_data():
            ...
            ball_angle = degrees(
                atan2(ball_data['direction'][1], ball_data['direction'][0]))
            ball_distance = abs(
                0.01666666 / (abs(ball_data['direction'][2]) / sqrt(1 - ball_data['direction'][2]**2)))
            self.xb = -sin(radians(ball_angle + self.heading)) * \
                ball_distance + self.xr  # x toop
            self.yb = cos(radians(ball_angle + self.heading)) * \
                ball_distance + self.yr  # y toop
            self.is_ball = True
        else:
            self.is_ball = False
    def move(self, xt, yt):
